...
function addUpdateMessages(messages) {
    ...
    if (
      ...
      logger.debug("Messages JSON: " + JSON.stringify(messages, null, 4));
  ...
  function __addUpdateMessage(message) {
    ...
    try {
      ...
      logger.debug(
        "Searching for messages matching the group and key, but not the value"
      );
      ...
      if (!messageSet.isEmpty() && messageSet.count() == 1) {
        logger.debug(
          "A message record was found matching the MSGGROUP and MSGKEY of the passed record. Updating the VALUE of the record."
        );
      ...
      logger.debug(
        "Update check complete. Determining if message should be added."
      );
      messageSet = MXServer.getMXServer().getMboSet(
        "MAXMESSAGES",
        MXServer.getMXServer().getSystemUserInfo()
      ...
      if (messageSet.isEmpty()) {
        logger.debug(
          "The message set returned 0 results when searching for the passed message. Adding new message."
        );
        //new message
  ...
  function __removeMessage(message) {
    ...
    try {
      ...
      if (!messageSet.isEmpty() && messageSet.count() == 1) {
        ...
        logger.debug(
          "Removing message with group=" +
            messageMbo.getString("MSGGROUP") +
            ", key=" +
            messageMbo.getString("MSGKEY") +
            ", value=" +
            messageMbo.getString("VALUE")
        );
        messageMbo.delete();
      } else {
        logger.debug(
          "removeMessage function found no matching messages. No action taken."
        );
      }
      messageSet.save();
    } finally {
