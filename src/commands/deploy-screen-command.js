...
export default async function deployScript(client, filePath, screen) {
    ...
    if (!screenName) {
        window.showErrorMessage("Unable to find presentation or systemlib id from current document. Cannot fetch screen from server to compare.", {
            modal: true
        });
        return;
    }

    await window.withProgress({ cancellable: false, title: "Screen", location: ProgressLocation.Notification }, async (progress) => {
