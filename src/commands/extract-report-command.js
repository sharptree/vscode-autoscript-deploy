...
export default async function extractReportCommand(client) {
    ...
    if (typeof extractLoc === 'undefined' || !extractLoc) {
        if (workspace.workspaceFolders !== undefined) {
            ...
            window.showErrorMessage(
                'A working folder must be selected or an export folder configured before exporting screen definitions.',
                {
                    modal: true,
                }
            );
    ...
    if (!fs.existsSync(extractLoc)) {
        window.showErrorMessage(
            `The report extract folder ${extractLoc} does not exist.`,
            { modal: true }
        );
        return;
    }

    let reportNames = await window.withProgress(
        {
            title: 'Getting report names',
    ...
    if (typeof reportNames !== 'undefined' && reportNames.length > 0) {
        ...
        await window
            .showQuickPick(mappedReportNames, {
                placeHolder: 'Search for report',
            })
            .then(async (reportName) => {
                if (typeof reportName !== 'undefined') {
                    await window.withProgress(
                        ...
                        async (progress, cancelToken) => {
                            ...
                            let reportInfo = await client.getReport(
                                report.reportId
                            );
        ...
        window.showErrorMessage('No reports were found to extract.', {
            modal: true,
        });
    }
}
...
export async function writeMetaData(reportInfo, extractLoc) {
    let xmlFilePath =
        extractLoc + '/' + reportInfo.reportFolder + '/reports.xml';
    ...
    if (
        typeof reportsXML.reports.report === 'undefined' ||
    ...
    if (reportInfo.resources) {
        const reportName = path.basename(
            reportInfo.reportName,
            path.extname(reportInfo.reportName)
        );

        let outputDir =
            extractLoc + '/' + reportInfo.reportFolder + '/' + reportName;
        report.resources = {};
        ...
        await new Promise((resolve, reject) => {
            yauzl.fromBuffer(
                ...
                (err, zipFile) => {
                    ...
                    zipFile.on('end', () => {
                        console.log(
                            `Extraction complete for report "${reportName}"`
                        );
                    });
                }
            );
