...
export default async function extractScreensCommand(client) {
    ...
    if (!fs.existsSync(extractLoc)) {
        window.showErrorMessage(
            `The screen extract folder ${extractLoc} does not exist.`,
            { modal: true }
        );
        return;
    }

    let screenNames = await window.withProgress(
        {
            title: 'Getting screen names',
    ...
    if (typeof screenNames !== 'undefined' && screenNames.length > 0) {
            ...
            .showInformationMessage(
                'Do you want to extract ' +
                    (screenNames.length > 1
                        ? 'the ' + screenNames.length + ' screen definitions?'
                        : ' the one screen definition?'),
                { modal: true },
                ...['Yes']
            )
            .then(async (response) => {
                            ...
                            await asyncForEach(
                                ...
                                async (screenName) => {
                                    if (!cancelToken.isCancellationRequested) {
                                        progress.report({
                                            increment: percent,
                                            message: `Extracting ${screenName.toLowerCase()}`,
                                        });
                                        let screenInfo = await client.getScreen(
                                            screenName
                                        );
        ...
        window.showErrorMessage(
            'No screen definitions were found to extract.',
            { modal: true }
        );
    }
}
