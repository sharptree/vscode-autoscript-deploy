            ...
            .showInformationMessage(
                'Do you want to extract ' +
                    (scriptNames.length > 1
                        ? 'the ' + scriptNames.length + ' automation scripts?'
                        : ' the one automation script?'),
                { modal: true },
                ...['Yes']
            )
            .then(async (response) => {
                            ...
                            await asyncForEach(
                                ...
                                async (scriptName) => {
                                    if (!cancelToken.isCancellationRequested) {
                                        ...
                                        let scriptInfo = await client.getScript(
                                            scriptName
                                        );
